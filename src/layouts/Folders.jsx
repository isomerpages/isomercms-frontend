...
const Folders = ({ match, location }) => {
  ...
  const initialMoveDropdownQueryState = `${folderName}${
  ...
  useEffect(() => {
    if (folderContents && folderContents.sha) {
      ...
      if (subfolderName) {
        const subfolderFiles = retrieveSubfolderContents(
          directoryFileOrder,
          subfolderName
        )
        if (subfolderFiles.length > 0) {
          setFolderOrderArray(
            subfolderFiles.filter((item) => item.fileName !== ".keep")
          )
        } else {
    ...
    <>
      {isFolderCreationActive && (
        <FolderCreationModal
          ...
          existingSubfolders={folderOrderArray
      ...
      {isFolderModalOpen && (
        <FolderModal
          ...
          siteName={siteName}
          folderType="page"
          existingFolders={folderOrderArray
            .filter((item) => item.type === "dir")
            .map((item) => item.fileName)}
        />
      )}
        ...
        <div className={contentStyles.mainSection}>
          ...
          <div className={contentStyles.contentContainerFolderRowMargin}>
            <FolderOptionButton
              ...
              option="rearrange"
              isDisabled={folderOrderArray.length <= 1 || !folderContents}
            />
            <FolderOptionButton
              title="Create new page"
              option="create-page"
              id="pageSettings-new"
              onClick={() => setIsPageSettingsActive((prevState) => !prevState)}
            />
            <FolderOptionButton
