 ...
 */
const ListItem = ({ item, onExpand, onCollapse, isDragging }) => {
  // Nested list
  ...
  switch (item.data.type) {
    ...
    default:
      return <p>Unaccounted for! {item.data ? item.data.type : "no"}</p>
  }
}
...
const readTree = (flattenedTree) => {
  ...
  for (let i = 1; i < flattenedTree.length; ++i) {
    ...
    switch (currDepth) {
      ...
      default:
        throw Error(
          "Tree contains invalid data - Isomer does not support items greater than depth level 4"
        )
    }
...
export { flattenTree, dataIterator, ListItem, draggableWrapper, readTree }
