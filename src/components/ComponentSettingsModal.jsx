...
const ComponentSettingsModal = ({
  ...
  const { data: resourceRoomName } = useQuery(
    ...
    () => getResourceRoomName(siteName),
    {
      ...
      onError: () => {
        errorToast(
          `The resource room name could not be retrieved. ${DEFAULT_RETRY_MSG}`
        )
      },
    }
  )
  ...
  useEffect(() => {
    setHasErrors(
      !isPost
        ? _.some(errors, (field) => field.length > 0) || !fileUrl
        : _.some(errors, (field) => field.length > 0)
    )
  }, [errors, fileUrl])
  ...
  useEffect(() => {
    setHasChanges(
      !(
        ...
        resourceDate === originalResourceDate
      )
    )
  ...
  const { mutateAsync: saveHandler } = useMutation(
    () => {
      ...
      if (fileName !== newFileName)
        return renamePageData(
          { siteName, resourceName: category, fileName, newFileName },
          newPageData,
          sha
        )
      return updatePageData(
        { siteName, resourceName: category, fileName },
        newPageData,
        sha
      )
    },
    {
      onSettled: () => {
        setSelectedFile("")
        setIsComponentSettingsActive(false)
      },
      ...
      onError: () =>
        errorToast(
          `${
            isNewFile
              ? "A new resource page could not be created."
              : "Your resource page settings could not be saved."
          } ${DEFAULT_RETRY_MSG}`
        ),
    }
  )
  ...
  const changeHandler = (event) => {
    ...
    const errorMessage = validateResourceSettings(
      id,
      value,
      pageFileNames.filter((file) => file !== fileName)
    )
        ...
        {(sha || isNewFile) && (
          <div className={elementStyles["modal-settings"]}>
            <div className={elementStyles.modalHeader}>
              <h1>
                {isNewFile ? "Create new resource page" : "Resource settings"}
              </h1>
              <button
                id="settings-CLOSE"
                type="button"
            ...
            <div className={elementStyles.modalContent}>
              <div className={elementStyles.modalFormFields}>
                ...
                <span className={elementStyles.infoGrey}>
                  Resources > {deslugifyDirectory(category)} >{" "}
              ...
              <SaveDeleteButtons
                isDisabled={
                  isNewFile ? hasErrors : !hasChanges || hasErrors || !sha
                }
